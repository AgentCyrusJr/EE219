function [A,Y,numIter,tElapsed,finalResidual]=wnmfrule(X,k,lamda,option)
...
for i=1:option.iter
    switch option.distance
        case 'ls'
           A=A.*(((W.*X)*Y')./(lamda*A + (W.*(A*Y))*Y'));
           A(A<eps)=0;
           A=max(A,eps);
           Y=Y.*((A'*(W.*X))./(lamda*Y + A'*(W.*(A*Y))));
           Y(Y<eps)=0;
           Y=max(Y,eps);
% %             Yt = Y';
% %            
...
% %             Y=max(Y,eps);
    ...
    if mod(i,10)==0 || i==option.iter
        ...
        curRes = sqrt(sum(sum(W.*(X-XfitThis).^2)) + lamda.*(sum(sum(A.^2)) + sum(sum(Y.^2))));
